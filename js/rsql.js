...
function apiType(colType) {
  ...
  var numTypes = ['int','bigint','smallint','integer','serial','bigserial',
    'real','float','double precision','decimal','number','numeric'];
...
function DatabaseManager(sqlEngine) {
  ...
  {
    this.sqlEngine.queryRows({'q' : queryViewNames,
                              'callback' : display,
                              'errback' : errback  });
  };
  ...
  {
    this.sqlEngine.queryRows({'q' : querySchemaNames,
                              'callback' : display,
                              'errback' : errback });
  };
  ...
  {
    this.sqlEngine.queryRows({'q' : queryFunctionNames,
                              'callback' : display,
                              'errback' : errback  });
  };
...
function ResourceMeta(tName) {
  ...
  this.getQuotedFunctionName = function() {
      ...
      if (args[0] === '')
          args = [parts[1]];
      quotedArgs = _.map(args, function(m) { return quoteIdentifier(m) });
